...
class NvidiaBackend(EngineBackend):
    ...
    def _effective_kv_cache_dtype(self) -> str:
        ...
        So the fp8 DEFAULT is downgraded to 'auto' for a multimodal model. An
        explicit non-fp8 ``kv_cache_dtype`` always wins — it isn't the default,
        so it passes through untouched (only the literal 'fp8' is overridable
        here). A model whose config.json can't be read keeps fp8.
        """
        dtype = getattr(self.config, "kv_cache_dtype", "") or ""
        if dtype == "fp8" and self._is_multimodal_model():
            return "auto"
        return dtype
