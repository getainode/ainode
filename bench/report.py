...
the file works opened from disk, pasted into an artifact host, or served by the
marketing site. Dark by default with a light palette under prefers-color-scheme.
...
def main():
    ...
    print(f"  {len(runs)} run(s) -> {out} ({kb:.0f} KB)")
    return 0
