...
A transcription model is the other kind that writes no tokens: audio goes in and it is
scored against words nobody typed at it. It is served here the way every other model is,
by vLLM on an engine image the catalog recipe pins, so it stacks beside a chat model on
one GPU and `POST /v1/audio/transcriptions` on any node routes to it by the model id in
the multipart body.

Three numbers decide whether one is usable. **WER** is the word error rate against the
exact text the clips were made from, pooled over words rather than averaged over clips,
with the stricter orthographic rate in brackets: the first folds number words to digits
so a transcript that heard every word and wrote "9" for "nine" is not called wrong for
its spelling, the second folds only case and punctuation, and the gap between them is
how much of the error was spelling rather than hearing. **Exact** counts the clips that
came back word for word, which is the number a user feels. **p50 ms** is one clip
uploaded on its own, end to end from wherever the bench ran, and **RTF** is the
real-time factor, wall seconds per second of audio, so below 1 means the engine
transcribes faster than the clip plays. The audio is ten committed WAVs
(`bench/speech/clips/`) in six voices across six English locales, every line carrying
digits and most a place or product name, because a wrong number and a wrong name are
where transcription goes wrong in a way a user notices. Generated from the same bench
records by [`scripts/render-bench-table.py`](scripts/render-bench-table.py); the block
is documented in [`bench/SCHEMA.md`](bench/SCHEMA.md).

<!-- BEGIN speech-bench-table (generated by scripts/render-bench-table.py) -->
