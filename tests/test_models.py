...
class _FakeHfHub:
    ...
    def __init__(self, files: dict, after_file=None):
        # files: {relative_path: bytes}
        self._files = files
        self._after_file = after_file
        self.downloaded = []

    def list_repo_files(self, repo_id, token=None):  # noqa: D401
        return list(self._files.keys())

    def hf_hub_download(self, repo_id, filename, local_dir, token=None):
