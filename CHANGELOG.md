...
- **The Hugging Face token stops travelling through job files and job logs.** It was serialized into each job's `config.json` and `config.container.json` (mode 644, in a directory the job API reads) and spelled out as `-e HF_TOKEN=<token>` in the launch line appended to the job log that `GET /api/training/jobs/{id}/logs` serves. Training, quantize and merge now pass it through a `--env-file` written 0600 and deleted when the container exits, the token is stripped from the config written to disk, and every launch line is logged through one scrubber that masks secret env values.
